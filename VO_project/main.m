...
rng(2);
[matched_p1, matched_p2, inliers_kpt, E] = bootstrap(img0, img1, K, 1000);
...
% looping
range = (bootstrap_frames(2) + 1) : last_frame; %last_frame;
% history array
...
for i = range
    ...
    [P_i, keep_P] = step(kptTracker, img);
    P_i = round(P_i)';
    keep_t = (P_i(1,:) < 1 | P_i(1,:) > width);
    keep_P(keep_t) = 0;
    keep_t = (P_i(2,:) < 1 | P_i(2,:) > height);
    keep_P(keep_t) = 0;
...
%     plot(new_P(1, :), new_P(2, :), 'yx', 'Linewidth', 2);
    ...
    [~, new_kpt, inliers_kpt, ~] = bootstrap(prev_img, img, K, 400);
    invalid_new_kpt = new_kpt((1-inliers_kpt) > 0, :);
    new_kpt = new_kpt(inliers_kpt, :)';
